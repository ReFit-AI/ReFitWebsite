...
import { verifyAndConsumeNonce, generateAndStoreNonce } from './nonce-storage.js';
...
export async function generateAuthChallenge() {
  const { nonce, expiresAt } = await generateAndStoreNonce();
 ...
 */
export async function verifySignature(publicKeyString, signature, message, nonce) {
  try {
    // Check nonce validity if provided
    if (nonce) {
      // Verify and consume the nonce (one-time use)
      const isValidNonce = await verifyAndConsumeNonce(nonce);
