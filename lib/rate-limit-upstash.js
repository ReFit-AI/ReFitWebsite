...
export async function rateLimit(request, type = 'api') {
  ...
  let result;
  try {
    result = await limiter.limit(fullIdentifier);
  } catch (error) {
    // If rate limiting fails (e.g., Redis unreachable), fail open
  ...
  // result.reset is a Unix timestamp in milliseconds (or Date for in-memory fallback)
