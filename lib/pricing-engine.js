...
export function calculateQuote({
  ...
  // We pay customer offerPrice, sell to supplier at wholesalePrice
  const profit = wholesalePrice - offerPrice;
  const minProfit = 15; // Minimum profit: $9 fixed costs (label + processing) + $6 margin

  // Reject if not profitable
  if (profit < minProfit || offerPrice < 1) {
    return {
      success: false,
      rejected: true,
      reason: 'This device cannot be accepted at current market prices',
      message: 'We cannot offer a competitive price for this device right now.'
    };
  }

  return {
    success: true,
    // Main price (USDC is primary)
    usdPrice: Math.round(offerPrice),
    usdcPrice: Math.round(offerPrice), // Explicit USDC amount
