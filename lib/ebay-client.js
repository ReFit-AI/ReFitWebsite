...
/**
 * Parse phone model and storage from an eBay listing title
 * e.g. "Apple iPhone 16 Pro Max 256GB Desert Titanium, Unlocked" → { model: "iPhone 16 Pro Max", storage: "256GB" }
 */
export function parseTitleForPhone(title) {
  if (!title) return { model: null, storage: null }

  // Extract storage (e.g. 64GB, 128GB, 256GB, 512GB, 1TB, 2TB)
  ...
  const iphoneMatch = title.match(/iPhone\s*(SE\s*(?:2nd|3rd|4th)?|\d{1,2}\s*(?:Pro\s*Max|Pro|Plus|Air|mini|e)?)/i)
  if (iphoneMatch) {
    const model = 'iPhone ' + iphoneMatch[1].replace(/\s+/g, ' ').trim()
    return { model, storage }
  }

  // Extract Samsung model
  const samsungMatch = title.match(/Galaxy\s*(S\d{2}\s*(?:Ultra|Plus|\+|FE)?|Z\s*(?:Fold|Flip)\s*\d*|Note\s*\d{1,2}\s*(?:Ultra|Plus|\+)?|A\d{2}s?)/i)
  if (samsungMatch) {
    const model = 'Galaxy ' + samsungMatch[1].replace(/\s+/g, ' ').trim()
    return { model, storage }
  }

  // Extract Google Pixel model
  const pixelMatch = title.match(/Pixel\s*(\d+\s*(?:Pro\s*XL|Pro|a)?)/i)
  if (pixelMatch) {
    const model = 'Pixel ' + pixelMatch[1].replace(/\s+/g, ' ').trim()
    return { model, storage }
  }

  return { model: null, storage }
}
...
export function normalizeOrder(ebayOrder) {
  ...
  // Seller is at top level SellerUserID, NOT inside Transaction.Item.Seller
  const sellerUsername = ebayOrder.SellerUserID || firstTx?.Item?.Seller?.UserID || ebayOrder.seller?.username || null
  ...
  // Tracking — check transaction-level ShippingDetails first (where eBay actually puts it),
  // then fall back to order-level
  const txShippingDetails = firstTx?.ShippingDetails?.ShipmentTrackingDetails
  const orderShippingDetails = ebayOrder.ShippingDetails?.ShipmentTrackingDetails
  const rawTrackingDetails = txShippingDetails || orderShippingDetails
  const trackingArray = Array.isArray(rawTrackingDetails) ? rawTrackingDetails : rawTrackingDetails ? [rawTrackingDetails] : []
  const firstTracking = trackingArray[0] || {}
  // Tracking numbers can be parsed as floats by JSON — force to string
  const rawTrackingNum = firstTracking.ShipmentTrackingNumber || ebayOrder.fulfillmentHrefs?.[0]?.trackingNumber || null
  const trackingNumber = rawTrackingNum ? String(rawTrackingNum).replace(/\.?\d+e\+?\d+/i, (m) => {
    // Convert scientific notation back to integer string
    try { return BigInt(Math.round(parseFloat(m))).toString() } catch { return m }
  }) : null
  const shippingCarrier = firstTracking.ShippingCarrierUsed ||
    ebayOrder.fulfillmentHrefs?.[0]?.shippingCarrier || null

  // Status mapping — eBay "Completed" means payment finalized, NOT delivered
  // Use ShippedTime to determine shipping status
  const ebayStatus = ebayOrder.OrderStatus || ebayOrder.orderFulfillmentStatus || ''
  let orderStatus = 'Active'
  if (/cancel/i.test(ebayStatus)) {
    orderStatus = 'Cancelled'
  } else if (ebayOrder.ShippedTime && trackingNumber) {
    orderStatus = 'Shipped'
  } else if (ebayOrder.ShippedTime) {
    orderStatus = 'Shipped'
  } else if (/complete|fulfilled/i.test(ebayStatus)) {
    orderStatus = 'Active' // Paid but not yet shipped
  }

  // Seller location — from item listing and originating postal code
  const sellerLocation = firstTx?.Item?.Location || null // e.g. "Rochester, New York"
  const sellerZip = firstTx?.ShippingDetails?.CalculatedShippingRate?.OriginatingPostalCode
    ? String(firstTx.ShippingDetails.CalculatedShippingRate.OriginatingPostalCode)
    : null
...
async function harvestContact(purchase) {
  // Parse seller location (e.g. "Rochester, New York" → city + state)
  let city = null, state = null
  if (purchase.seller_location) {
    const parts = purchase.seller_location.split(',').map(s => s.trim())
    if (parts.length >= 2) {
      city = parts[0]
      state = parts[1]
    } else {
  ...
  if (existing) {
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        // Auto-fill location if not manually set
        ...(city && !existing.city ? { city } : {}),
        ...(state && !existing.state ? { state } : {}),
        ...(purchase.seller_zip && !existing.zip ? { zip: purchase.seller_zip } : {}),
