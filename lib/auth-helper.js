  ...
  try {
    ...
    if (useServerAuth || hasProblematicExtensions()) {
      ...
      if (response.ok) {
        ...
        // Set the session in the client (skip if no session returned)
        if (data.user && data.session && supabase) {
          // Only try to sign in if we have a session to set
          try {
            if (data.session) {
              await supabase.auth.setSession(data.session);
            }
          } catch (signInErr) {
            console.warn('Failed to set session, continuing anyway:', signInErr.message);
          }
        }
